 ...
 * @author pc
 */
public class AdminTransferFail extends HttpServlet {

    private BalanceHandler balanceHandler;

    public AdminTransferFail() {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        int amountToDeduct = moneyInt - moneyInt*10/100;
        ...
        if (statusOfOrderId.equalsIgnoreCase("1")) {
            userWithdrawalDAO.updateStatusRequest(Integer.parseInt(orderId), 3);
            balanceHandler.addToQueue(uid, amountToDeduct);
            jsonResponse.put("message", "Từ chối chuyển tiền, do thông tin chuyển khoản sai.");
        } else {
            jsonResponse.put("message", "Bạn đã từ chối chuyển khoản rồi!");
        }
